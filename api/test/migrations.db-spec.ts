...
describe('Migrations + seed config (Postgres thật)', () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      'DROP TABLE IF EXISTS chat_messages, chat_conversations, catalog, department, outbox, ticket_file, booking, ticket, inventory_round_files, inventory_rounds, files, asset_note, allocation_history, assets, sessions, audit_log, users, config, _migrations CASCADE',
    );
  });
  ...
  it('chạy đủ migration theo thứ tự tên file', async () => {
    ...
    expect(applied).toEqual([
      ...
      '0049_booking_due_notified.sql',
      '0050_users_department.sql',
      '0051_software_unique_per_machine.sql',
    ]);
  });
  ...
  it('bảng config seed đủ 10 tham số FR-44 đúng giá trị mặc định', async () => {
    ...
    expect(byKey).toEqual({
      ...
      // 0041 (audit H-2): trần thời lượng 1 lượt mượn = 90 ngày
      max_booking_duration_hours: 2160,
    });
  });

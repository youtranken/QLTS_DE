...
function Wheel({ count, render, value, onChange, ariaLabel }: WheelProps) {
  ...
  useEffect(() => {
    ...
    // Kéo-thả bằng CHUỘT để cuộn (touch/trackpad vẫn dùng native pan-y). Trong lúc kéo tắt
    // scroll-snap để không giằng, thả thì bật lại + glide về nấc gần nhất. `moved` chặn click
    // sau kéo bị hiểu thành "click ô" (nhảy sai nấc).
    let dragging = false;
    let startY = 0;
    let startTop = 0;
    let moved = false;
    ...
    const up = (e: PointerEvent) => {
      ...
      glide(Math.round(col.scrollTop / ITEM_H));
    };

    const click = (e: MouseEvent) => {
      if (moved) {
        moved = false; // vừa kéo xong → bỏ qua click ô này
        return;
      }

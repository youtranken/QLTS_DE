...
import { CodeCell, TypeCell } from '@/features/assets/asset-cell';
...
export function useAssetColumns(opts: {
  ...
  return useMemo<ColumnDef<AssetRow, unknown>[]>(() => {
    const statusCol: ColumnDef<AssetRow, unknown> = {
      ...
      // Status + Action bám mép PHẢI bảng (căn phải header + ô).
      meta: { className: 'num' },
      cell: ({ row }) => (
    ...
    const deviceCols: ColumnDef<AssetRow, unknown>[] = [
      ...
      {
        id: 'employeeCode',
        header: th(t('assets.col.employeeCode'), 'Mã nhân viên của người giữ (theo IDP)'),
        enableSorting: false,
        cell: ({ row }) => row.original.employeeCode ?? '—',
      },
      {
        id: 'department',
        header: th(t('assets.col.department'), 'Phòng ban của người giữ (theo IDP)'),
        enableSorting: false,

      ...
      {rows.length === 0 ? (
        ...
        <div className="table-wrap">
          <table className="table">
            ...
            <tbody>
              {rows.map((row) => {
                // Tới hạn trả trong hôm nay (chưa quá hạn) — chỉ ý nghĩa với hàng ĐANG MƯỢN.
                const dueToday =
                  !row.isOverdue &&
                  kind === 'return' &&
                  !!row.to &&
                  new Date(row.to).toDateString() === new Date().toDateString();
                return (
                <tr
                  key={row.id}
                  className={
                    row.isOverdue ? 'overdue' : dueToday ? 'due-today' : undefined
                  }
                >
                  <td>
                    {row.borrowerName ?? '—'}
                    {/* F8: badge quá hạn NGAY trên queue (3.8/3.12 AC "đỏ, sort"). */}
                    {row.isOverdue && (

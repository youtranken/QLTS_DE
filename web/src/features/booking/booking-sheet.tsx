...
export function BookingSheet({
  ...
  // Ngày máy đã BẬN (YYYY-MM-DD giờ VN) → khóa trên DatePicker để không chọn trùng.
  ...
  // Nạp các ngày máy đã bận (60 ngày tới) khi đổi máy → khóa ngày trùng trên lịch chọn.
  useEffect(() => {
    if (!assetId) {
    ...
    const toIso = new Date(Date.now() + 60 * 864e5).toISOString();
    fetch(
      `/api/booking/machines/${assetId}/busy-dates?from=${encodeURIComponent(fromIso)}&to=${encodeURIComponent(toIso)}`,
      { signal: c.signal },
    )
      .then((r) => (r.ok ? (r.json() as Promise<string[]>) : []))
      .then((days) => setBusyDates(new Set(days)))
      .catch(() => undefined);
    return () => c.abort();
  }, [assetId]);

...
/**
 * Các ngày YYYY-MM-DD của `weekday` (ISO 1=T2..7=CN) từ HÔM NAY tới cuối THÁNG hiện tại (giờ VN).
 * Định kỳ chọn 1 thứ → sinh đúng các ngày thứ đó trong tháng (vd 3/8 chọn T2 → 3,10,17,24,31).
 */
export const monthlyWeekdayDates = (
  weekday: number,
  base = todayLocal(),
): string[] => {
  const [y, mo, d0] = base.split('-').map(Number);
  const last = new Date(y, mo, 0).getDate(); // ngày cuối tháng mo (mo 1-based → new Date(y,mo,0))
  const pad = (n: number) => String(n).padStart(2, '0');
  const out: string[] = [];
  for (let d = d0; d <= last; d++) {
    const iso = `${y}-${pad(mo)}-${pad(d)}`;
    if (isoDow(iso) === weekday) out.push(iso);
  }
  return out;
};
...
export interface FreeMachine {
  ...
  /** Tên phần mềm đang cài (gộp ", ") — hover xem nhanh thay cho cấu hình. */

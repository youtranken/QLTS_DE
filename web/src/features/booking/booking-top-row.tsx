...
export function BookingTopRow({
  ...
  const machineOptions = (
    q
      ? poolList.filter((m) =>
          `${m.code} ${m.type} ${m.software ?? ''}`.toLowerCase().includes(q),
        )
      : poolList
  ).slice(0, 50);
  // Hover: liệt kê phần mềm của máy (thay cấu hình). Chuỗi rỗng → chưa cài.
  const swTitle = (m: FreeMachine) =>
    m.software
      ? `${t('bookingSheet.software', 'Phần mềm')}: ${m.software}`
      : t('bookingSheet.noSoftware', 'Chưa cài phần mềm');
  return (
      ...
      <div className="topcell">
        ...
        {editMachine || !selected ? (
          <Combobox
            ...
            getKey={(m) => m.id}
            renderOption={(m) => (
              // .combo-option là flex-column: giữ mono + small là 2 con trực tiếp (2 dòng);
              // title trên cả 2 dòng để hover đâu cũng thấy phần mềm.
              <>
                <span className="mono" title={swTitle(m)}>
                  {m.code}
                </span>
                <small title={swTitle(m)} style={{ cursor: 'help' }}>
                  {m.type}
                </small>
              </>
            )}
          ...
          <div
            className="picked-machine compact"
            title={swTitle(selected)}
          >
            <AssetTypeIcon type={selected.type} />
            <div className="pm-info">
              <span className="mono pm-code">{selected.code}</span>
              <span className="muted pm-spec">{selected.type}</span>
            </div>
            <button
              type="button"
              className="ghost sm"
